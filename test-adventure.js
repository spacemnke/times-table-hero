...
(async () => {
  ...
  // seed a profile with a small daily goal (fewer gates = faster test)
  ...
  console.log("✓ adventure started — hero =", await page.textContent('#ps-name'), "| gates:", total);
  ...
  while (solved < total && Date.now() < deadline) {
    ...
    for (const ch of String(q.a * q.b)) await page.click(`#adv-keypad .key[data-k="${ch}"]`);
  ...
  await page.waitForSelector('#adv-win:not([hidden])', { timeout: 10000 });
  console.log("✓ reached castle:", (await page.textContent('#adv-win-title')).trim(), "|", (await page.textContent('#adv-win-msg')).trim(), "| stars", (await page.textContent('#adv-win-stars')).trim());

  // verify it fed the REAL engine
  const p = await page.evaluate(() => JSON.parse(localStorage.getItem("tth.progress.v2.mia")));
  console.log("✓ engine fed → xp:", p.xp, "| coins:", p.coins, "| totalCorrect:", p.totalCorrect, "| facts:", Object.keys(p.facts).length, "| todayQ:", (Object.values(p.days)[0] || {}).q);
  ...
  // back home, streak should now be 1 (daily goal 10 met)
  await page.click('#adv-win-home');
  await page.waitForSelector('.screen--home.is-active');
  console.log("✓ home — streak:", (await page.textContent('#streak-num')).trim(), "| xp:", (await page.textContent('#xp-text')).trim());
