...
(async () => {
  ...
  const sec = await page.$('.lp-hgrid');
  if (sec) { await sec.screenshot({ path: path.join(root, 'shot-heroes.png') }); }
  else { await page.screenshot({ path: path.join(root, 'shot-heroes.png'), fullPage: false }); }
