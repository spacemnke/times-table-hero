...
(async () => {
  ...
  const sec = await page.$('.lp-steps');
  if (sec) { await sec.screenshot({ path: path.join(root, 'shot-how.png') }); }
  else { await page.screenshot({ path: path.join(root, 'shot-how.png'), fullPage: false }); }
